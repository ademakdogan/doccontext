...
from doccontext.models.job import JobStatus
...
def settings() -> Settings:
    s = Settings(
        rabbitmq_document_jobs_queue=f"doccontext_test_idx_{uuid.uuid4().hex[:8]}",
    )
    missing: list[str] = []
...
async def _drain_one_message(settings: Settings, timeout: float = 5.0) -> dict:
    """Read exactly one message off the configured queue for assertions."""
    import aio_pika
...
async def test_index_document_creates_queued_job_and_publishes_envelope(
    settings: Settings,
    repo: JobRepository,
    grpc_channel: grpc.aio.Channel,
) -> None:
    stub = pb_grpc.DocContextStub(grpc_channel)
    resp = await stub.IndexDocument(
        pb.IndexDocumentRequest(
            document_id="doc-123",
            client_id="tenant-1",
            user_id="u-1",
            corpus_id="corpus-a",
    ...
    assert envelope["job_type"] == "INDEX_DOCUMENT"
    assert envelope["document_id"] == "doc-123"
    assert envelope["file_type"] == "pdf"


async def test_index_document_generates_document_id_when_omitted(
    settings: Settings,
    repo: JobRepository,
    grpc_channel: grpc.aio.Channel,
) -> None:
    ...
    job = await repo.get(resp.job_id)
    assert job is not None
    assert job.document_id  # uuid string, non-empty
    await _drain_one_message(settings)  # clear the queue
...
@pytest.mark.parametrize(
    ...
    [
        (
            dict(corpus_id="c", file_type=pb.PDF, storage_path="/x"),
            "client_id is required",
        ),
        (
            dict(client_id="t", file_type=pb.PDF, storage_path="/x"),
            "corpus_id is required",
        ),
        (
            dict(client_id="t", corpus_id="c", file_type=pb.PDF),
        ...
        (
            dict(
                client_id="t",
                corpus_id="c",
                file_type=pb.FILE_TYPE_UNSPECIFIED,
                storage_path="/x",
            ),
            "unsupported",
        ),
...
async def test_index_document_rejects_invalid_requests(
    grpc_channel: grpc.aio.Channel, kwargs, expected_detail
) -> None:
    stub = pb_grpc.DocContextStub(grpc_channel)
    with pytest.raises(grpc.aio.AioRpcError) as ei:
        await stub.IndexDocument(pb.IndexDocumentRequest(**kwargs))
    assert ei.value.code() == grpc.StatusCode.INVALID_ARGUMENT
    assert expected_detail in ei.value.details()
